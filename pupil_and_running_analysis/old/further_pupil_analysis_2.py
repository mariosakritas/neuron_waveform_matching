...
@click.command(name='poster')
@click.argument('db_path', type=click.Path(exists=True))
@click.option('--mouse', '-m', default='M10205')
@click.option('--output', '-o', default=None)
def cli_poster(db_path=None,
               mouse=None,
               output=None):

    # I've changed the order of this list- hopefully it doesn't matter(?)
    conditions = ['Const', 'Reg', 'Rand', 'Step', 'RegRand', 'RandReg']
    ...
    # the database path is provided as argument (db_path) so use relateive
    # paths to make sure things are portable
    ...
    for i, alignment in enumerate(['onset', 'transition']):
        ...
        for k, what in enumerate(['Still', 'Running']):
            ...
            for j, cond in enumerate(conditions):
                ...
                if cond in results_cond:
                    ...
                    for k in range(n_trials):
                        before = np.nanmean(trials[k, :n_pre])
                        after = np.nanmean(trials[k, n_pre:2*n_pre])
                        R[k] = (after - before) * 100 / before

                    n_valid = np.sum(~np.isnan(R))
                    R = R[~np.isnan(R)]
                    y = np.mean(R)

                    # --- plot data points for this condition ---

                    # single data points
                    ax.plot(x0 * np.ones_like(R), R, 'o',
                            color=3 * [.4],
                            ms=3,
                            mec='none',
                            zorder=-1)
                    ...
                    # bar does not allow to adjust thickness of error caps
                    # so use the errorbar function
                    ax.errorbar([x0], y, np.std(R) / np.sqrt(n_valid),
                                capsize=5,
                                elinewidth=2,
                                capthick=1.5,
                                ecolor=3*[0])[1]
            ...
            ax.set_xlabel('')
            ax.set_xlim(-.5, len(cond_labels) - .5)
    ...
    if output is not None:
        # save figure to file if output directory is given

        if not op.exists(output):
            os.makedirs(output)

        fig_file = op.join(output, 'randreg_pupil_analysis_conditions.')
        for ff in ['png', 'pdf', 'svg']:
            fig.savefig(fig_file + ff,
                        format=ff,
                        dpi=600)
